...
% %% Track Power
% for n = 1:100
%     power_est = estimate_meanSNR(power,n);
%     ind = ~isnan(power_est);
%     MSE(n) = mean((power_est(ind)-power(ind)).^2);
% end
% [~,ind] = min(MSE);
% power_est = estimate_meanSNR(power,ind);
% 
% %% Remove Initial Transient
% power_est = power_est(ind+1:end);
% power = power(ind+1:end);
% tMeas = tMeas(ind+1:end);
% plot(power_est./PowerMax)
% hold off